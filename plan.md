...
## Observatory paragraph (big-bang unified Buffer pilot)
Buffer is now natively supported via the unified architecture (IGeometrySegment + GetSegments() + GeometryOperationDispatcher/BufferOp). One code path iterates the segment list for *all* input geometry kinds (Linear/CircularString, CompoundCurve, Polygon, CurvePolygon). When any CircularArcSegment is present the analytical leaves (ARC_OFFSET_XY homothety from proofs, CurveRingOffset segment-wise, round joins/caps, CurveBufferArea algebra) are used directly; no Linearize/Flatten fallback and no per-type branches. Output type is CurvePolygon precisely when the input carried arcs (with proper hole/collapse/endcap rules); pure-linear inputs continue to hit the legacy path (zero regression). The proofs/oracle side already had the segment model at ring level (CurveSegment) and offset assembly; the big-bang lifts it to top-level Geometry segments in NTS with dispatcher. Buffer matrix row now driven by # coverage: tags (Arc/CS/CC/CP ⚠️ via red_buffer_unified + region tests; Multi ❌). Dovetailed with dashboard PR #274. RGR: extensive Read of oracle BUFFER_REGION/ARC_BUFFER_SIMPLE + gens + Curve*Offset + Buffer*.v theories, Red failing tests for open CS + CP compound cases, Green unified wiring + NTS sketch in docs, Refactor clean + verification on linear gens. See red_buffer_unified_tests.py, updated driver BUFFER_UNIFIED, arc-offset-red-test-example.cs for IGeometrySegment + dispatcher, and TRIAGE update.
