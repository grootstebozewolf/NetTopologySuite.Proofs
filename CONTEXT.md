...
geodesic letter. Famous Science/arXiv 1804.07389 endpoints are the
famous-geodesicstring letter. Unknown well-formed CS is the angles
letter, not leftover Decline. Both CLOTHOID forms are the MkClothoid
letter.
...
**Famous geodesicstring** (ADR-0007, claimId `0007-famous-geodesicstring`):
Two locked WKT fixtures from Chabukswar & Mukherjee,
arXiv:1804.07389 (Science news: longest ocean / land straight
paths). Endpoints DMS → decimal lon lat, OGC order.
...
Each is tagged CST `TGeodesicString pts`; μ → `IntakeBag` with
only `MkChord` chickens — same bag as the matching `LINESTRING`
(`IntakeFamousGeodesic.v : ticket_0007_famous_geodesicstring_qed_or_qex`).
τ is LINESTRING; `cst_prod_tag` is None; no `TagGeodesic`; κ does
not gain 13. Empty / singleton still `ID_Empty` / `ID_BadPointCount`;
spiral still Decline. Named QEX park (do not discharge): Earth
great-circle length (~32090 km / ~11241 km) or angular span
(~288°35′ / ~101°6′); ETOPO1 land/water mask / “longest
uninterrupted”; sphere vs WGS84 ellipsoid / geoid; optimality of
branch-and-bound; emit of GEODESICSTRING bytes / WKB 13 as signed
I/O (`IntakeFamousGeodesic.v : famous_earth_length_missing`,
`IntakeFamousGeodesic.v : famous_etopo1_missing`,
`IntakeFamousGeodesic.v : famous_sphere_vs_wgs84_missing`,
`IntakeFamousGeodesic.v : famous_branch_bound_missing`,
`IntakeFamousGeodesic.v : famous_emit_wkb13_missing`).
Fixtures + honesty on existing μ (`0007-intake-geodesic`). Not a
new cook. Not a remint of #744 / #745.
_Avoid_: MkGeodesic, we proved the Science path length, silent
chord demote as Earth geodesic, first-cook expand, WKB 13, emit
