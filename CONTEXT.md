...
Total function `first_slice_tag` on host eggs. Named tags are SFA
1–7 and signed-curve 8–12 only. HOLD is codes {13..17, 18..21},
not named constructors. `MkChord` → LINESTRING; every `MkCirc` →
CIRCULARSTRING (CIRCLE CST is the same egg — intake
`ogc_iso_circle_same_egg`); `MkClothoid` / `MkOutOfScope` → None.
COMPOUNDCURVE is a bag, not an egg tag. Mode D joints stay
`cloth_joint` / `circ_split_join`. Emit stays QEX.
_Avoid_: factory-as-Qed, Circle-as-18, dual inhabit, HOLD name
fanfic, new oracle keyword
