...
> The only exceptions are the seven registered `Admitted` entries (6 counterexamples in `admitted-counterexamples.txt`, 1 deferred proof in `admitted-deferred-proofs.txt`), each with a concrete seam on file and discharge plan.
