...
> The only exceptions are the six registered `Admitted` entries (3 counterexamples in `admitted-counterexamples.txt`, 3 deferred proofs in `admitted-deferred-proofs.txt`), each with a concrete seam on file and discharge plan.
