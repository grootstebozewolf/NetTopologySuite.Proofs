...
(*                                                                            *)
(* Future direction (sibling-corpus target).  The clothoid paper's            *)
(* monotone-branch precondition `|kappa_i * L| <= pi` is connected to        *)
(* `f'(L) > 0` via a continuous-turning monotonicity argument that "lives    *)
(* naturally in Azimuth.v" (paper's wording, Section 7).  That bridge        *)
(* lemma is the next concrete Azimuth.v target if cross-corpus formalisation *)
(* of the Halley termination model is taken up.                              *)
(* -------------------------------------------------------------------------- *)
