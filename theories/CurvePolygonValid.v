(* ============================================================================
   ...
   the three control points `[start; mid; end]` (the exact Option-B
   approximation; the n-chord trigonometric refinement is deferred), the bridge
   is the EXACT rational CONTROL POLYGON -- so `Overlay.point_in_ring`
   (ray-parity) and `Overlay.hole_inside_outer` on it are exact, no trig.

   Defined here (all THREE-AXIOM, exact):
     §1  `point_in_curve_ring p r n` := p is inside the densified ring
         (`point_in_ring p (chord_approx_ring r n)`) -- the predicate the
         exact-Q POINT_IN_CURVE_RING oracle decides.
     ...
     §3  Projections: a CP-valid polygon is simple and, for each hole, has a
         hole vertex inside the densified outer ring
         (`valid_curve_polygon_cp_hole_witness`) -- the per-hole fact the oracle
         pins by deciding POINT_IN_CURVE_RING of a hole vertex against the shell.

   DEFERRED (honest scope): this is CONTROL-POLYGON (chord-approx) containment --
   sound up to the arc bulge; the refined n-chord densification and the
   "every hole point inside" Jordan strengthening of `hole_inside_outer`
   (Overlay flags it as non-Milestone-1) are future.  Sector orientation and
   holes mutually disjoint are the remaining CP_VALID slices.
...
(* -------------------------------------------------------------------------- *)
(* §1  Point inside a curve ring (via the densified control polygon).          *)
(* -------------------------------------------------------------------------- *)

Definition point_in_curve_ring (p : Point) (r : CurveRing) (n : nat) : Prop :=
...
Theorem valid_curve_polygon_cp_hole_witness :
  forall (cp : CurvePolygon) (n : nat) (h : CurveRing),
    valid_curve_polygon_cp cp n -> In h (curve_holes cp) ->
    exists p : Point,
      In p (chord_approx_ring h n) /\ point_in_curve_ring p (curve_outer cp) n.
Proof.
