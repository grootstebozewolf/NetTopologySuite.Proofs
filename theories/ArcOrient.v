...
(* Rotation invariance: rotating all four points by the same angle (c,s) with
   c^2 + s^2 = 1 leaves inCircle_R unchanged.  Geometrically the augmented 3x3
   transform [[R,0],[0,1]] has determinant det(R) = c^2 + s^2 = 1, so the
   lifted determinant is preserved.  With translation + scaling above, the
   inCircle SIGN is thus invariant under every orientation-preserving
   similarity -- the geometric basis of the Delaunay test.  (nsatz: the
   identity holds modulo the ideal generated by c^2 + s^2 - 1.) *)
Lemma inCircle_R_rotation_invariant : forall (A B C P : Point) (c s : R),
  c * c + s * s = 1 ->
  inCircle_R (mkPoint (c * px A - s * py A) (s * px A + c * py A))
             (mkPoint (c * px B - s * py B) (s * px B + c * py B))
             (mkPoint (c * px C - s * py C) (s * px C + c * py C))
             (mkPoint (c * px P - s * py P) (s * px P + c * py P))
  = inCircle_R A B C P.
Proof. intros A B C P c s Hcs. unfold inCircle_R. cbn. nsatz. Qed.
...
Print Assumptions inCircle_R_rotation_invariant.
