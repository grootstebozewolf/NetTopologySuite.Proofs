...
| **N-AA** | Arc-arc noding / intersection | #64 | `ArcArcCircles.v`, `ArcArcSound.v`, `ArcIntersect.v` (`arc_arc_intersects` predicate); `CircularCookZ.v : I_circles_z` (integer discriminant + hen mint, claimId `64-i-circular`); oracle `ARC_ARC_XY` / `I_CIRCULAR` | 🟡 circles-intersect (Stage B) ✅; Z classifier Hit/Empty/Touch/Decline extracted ✅; γ `[0,1]` / span filter / glossary 𝓘 open |
