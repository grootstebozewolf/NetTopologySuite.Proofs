...
| **N-AA** | Arc-arc noding / intersection | #64 | `ArcArcCircles.v`, `ArcArcSound.v`, `ArcIntersect.v` (`arc_arc_intersects` predicate); `CircularCookZ.v : I_circles_z` (integer discriminant + hen mint, claimId `64-circ-z-partition`); oracle `ARC_ARC_XY` / `I_CIRCULAR` | 🟡 circles-intersect (Stage B) ✅; Z classifier partition iff + internal kiss ✅; γ `[0,1]` / span filter / glossary 𝓘 open |
