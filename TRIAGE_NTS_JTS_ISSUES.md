...
**Per-ask status (8 asks from issue body, accurate as of 2026-06):**
- #1 Snap-rounding passes-through: ✅ (tight grid) / 🟡 (full `2²⁵`)
- #2 Precision reducer idempotence: ✅
- #3 OverlayNG boolean semantics: ✅
- #4 OverlayNG end-to-end correctness: 🟡 (conditional headline)
- #5 DCEL ring assembly: 🟡 (planar Euler hyps in extract_rings_valid)
- #6 Fixed-precision hole collapse (JTS#979): ✅ mechanism
- #7 Curve snap decision (PRC-SN): ✅
- #8 Oracle / extraction: 🟡 (many extracted, interface-boundary for transcendentals)
Remaining gaps explicitly named in docs/snap-rounding-rgr-pivot.md, verified-claims, issue #66 body (C1 width, C2 parked, unconditional OV, arc Hobby). C1 tight regime closed per `PassesThrough_b64_grid_exact.v`. | Keep Urgent — largely delivered, closing gaps |
