...
| **#67** | RelateNG / 9IM matrix & boundary handling | `Immediate` | **DE-9IM suite through S15l in the working tree.** Per `docs/issue-67-relateng-triage.md` + `docs/verified-claims.md`: matrix algebra + witnesses (S0–S12), prepared-cache refinement (S13–S14b), **line×line noding** (S15a–k — `RelateNodingLineLine.v` collection capstone), and **triangle touch capstone (S15l — PR #263):** `touch_triangle_pair_strict_ii_no_common` (no shared strict interior, Qed via `g_sum` + `nra` on 18 `shares_edge` cases), `tri_bnd_v1/v2/v3` + `touch_triangle_pair_bb_cell` (shared vertex on both boundaries, Qed), `touch_triangles_satisfy_pointset` + `_and_general` (real `~II∧BB∧EE` conjunction, Qed). Integer substrate: `RelateIntDetBound.v` closes `idet_abs_le_sq` + `idet_fits_int64_for_cmax_coords` (0 axioms). Curve DE-9IM oracle `CURVE_RELATE_MATRIX`. **Remaining (S15l+ open):** `touch_triangle_pair_ii_cell` (JCT/point_set seam), `touch_triangle_f_cells_trimmed` (lift details), `relate_triangle_touch` (classify dispatch stub); prepared evaluate hook; Jordan/overlay cell-dimension soundness; multi-geometry pipeline. JTS#1175 fixed upstream (jts#1200). | Keep Immediate — S15l triangle touch capstone landed (PR #263); JCT seam + Jordan soundness remain |
...
| **BUF-1 / BUF-N** | Single-/multi-arc buffer → `CurvePolygon` | #65 | 18× `Buffer*.v` (linear), `ExtractBufferRings.v`; `CurveBufferArea.v`, `CurveRingOffset.v`, `CurveOffsetAssembly{,Total}.v`, `CurveRoundJoin.v`; oracle `BUFFER_REGION` (+ `ARC_BUFFER_SIMPLE/FULL` pins) | 🟡 arc buffer-region boundary+area certificate ✅ (3-ax); Minkowski-area soundness deferred; **+ d=0 identity + helpers (RelateCurveArcSegment.v: arc_buffer_simple_d0_is_identity, arc_offset_arc_0, etc.)** |
