...
| **N-AA** | Arc-arc noding / intersection | #64 | `ArcArcCircles.v`, `ArcArcSound.v`, `ArcIntersect.v`; `CircleCircleResultant.v`; `CircularCookZ.v` / Hit / Span / sidecar cook (`I_ok_circ`, `I_ok_mixed`); oracle `ARC_ARC_XY` / `I_CIRCULAR`. Ticket citations: [ADR-0007](docs/adr/ADR-0007-sheet-hen-cook-noding-model.md) Parks + [`docs/verified-claims.md`](docs/verified-claims.md) | 🟡 Parks Γ / ι / ρ named QEX (ADR-0007 Parks). Circles-intersect + radical / Z / Hit / span / sidecar cook ✅ (Campaign I–II + Phase B joints/bags). Host CircGamma QEX; first cook chord–chord. `I_ok_circ` / `I_ok_mixed` Hit ≠ host `I_ok`. |
