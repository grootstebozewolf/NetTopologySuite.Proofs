...
| **BUF-1 / BUF-N** | Single-/multi-arc buffer → `CurvePolygon` | #65 | 18× `Buffer*.v` (linear), `ExtractBufferRings.v`; `CurveBufferArea.v`, `CurveRingOffset.v`, `CurveOffsetAssembly{,Total}.v`, `CurveRoundJoin.v`; oracle `BUFFER_REGION` (+ `ARC_BUFFER_SIMPLE/FULL` pins) | 🟡 arc buffer-region boundary+area certificate ✅ (3-ax); Minkowski-area soundness deferred; **+ d=0 identity + unsafe radius (RelateCurveArcSegment.v)** |
