...
| **N-AA** | Arc-arc noding / intersection | #64 | `ArcArcCircles.v`, `ArcArcSound.v`, `ArcIntersect.v` (`arc_arc_intersects` predicate); **`CircularCookZ.v : I_circles_z`** / **`CircularCook.v : I_circles_on_z_sheet`** (Year-1 circular 𝓘, claimId `64-i-circular`); oracle `ARC_ARC_XY` / **`I_CIRCULAR`** | 🟡 circles-intersect (Stage B) ✅; **extracted 𝓘 Hit/Empty/Decline + hen mint** ✅ (not OverlayNGCurve wire, not `fully_intersected` retirement, not the #671 resultant); quartic coordinates / γ `[0,1]` / span filter open |
