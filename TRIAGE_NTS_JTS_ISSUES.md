...
| **N-AA** | Arc-arc noding / intersection | #64 | `ArcArcCircles.v`, `ArcArcSound.v`, `ArcIntersect.v` (`arc_arc_intersects` predicate); `CircularCookZ.v : I_circles_z` (integer discriminant + hen mint, claimId `64-circ-z-partition`); `CircularCookHit.v : ticket_64_circ_hit_params_qed_or_qex` (full-circle `γ` + `(tᵢ, tⱼ)`, claimId `64-circ-hit-params`); oracle `ARC_ARC_XY` / `I_CIRCULAR` | 🟡 circles-intersect (Stage B) ✅; Z classifier partition iff + internal kiss ✅; full-circle locked Hit `(h*, p*, tᵢ, tⱼ)` ✅; CircularArc span / `CircularCook.v : ticket_64_circ_gamma_qed_or_qex` QEX |
