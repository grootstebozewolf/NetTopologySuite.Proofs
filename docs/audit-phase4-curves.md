...
**Update (2026-06-12).** That target has since landed as [`theories/ClothoidResidual.v`](../theories/ClothoidResidual.v) — the monotone-branch uniqueness theorem, CONDITIONAL (Qed) with the derivative facts as named Section hypotheses witnessed by `clothoid-halley-coq/coq/Clothoid_L.v`. The companion corpus now has a **public repro** at the §6.1 URL, **relicensed to the standard EUPL-1.2** (data files CC BY 4.0; verified against the raw LICENSE, 2026-06-12), with a README note providing the proofs *"for integration into formal geometry corpora such as NetTopologySuite.Proofs (itself BSD-3-Clause licensed and governed exclusively by its own licence)"* — superseding the proprietary-licence description in §6.1 above, which documents the pre-publication state. The three remaining open questions of the clothoid lane (full Fresnel support, exact integer-parameter regime, performance-vs-linearisation) are triaged in [`clothoid-open-questions-triage.md`](clothoid-open-questions-triage.md).
