...
**Update (2026-06-12).** That target has since landed as [`theories/ClothoidResidual.v`](../theories/ClothoidResidual.v) — the monotone-branch uniqueness theorem, CONDITIONAL (Qed) with the derivative facts as named Section hypotheses witnessed by `clothoid-halley-coq/coq/Clothoid_L.v`. The companion corpus now has a **public repro** at the §6.1 URL (witness proofs compilable by anyone; LICENSE unchanged — source-available, derivative use still gated). The three remaining open questions of the clothoid lane (full Fresnel support, exact integer-parameter regime, performance-vs-linearisation) are triaged in [`clothoid-open-questions-triage.md`](clothoid-open-questions-triage.md).
