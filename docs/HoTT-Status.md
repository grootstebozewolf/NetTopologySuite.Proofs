...
| Shewchuk base (orient/expansion) | GREEN skeleton (PR #90) + fill slice (feature/hott-rgr-shewchuk-fill-proofs): formal_orient + degenerate Qed + cross_antisym lemma + shewchuk alias | `theories-hott/ShewchukBaseEquiv.v` (Point + cross + cross_antisymmetric (re-expressed), Dir, formal_orient defined (Rlt/Rgt_dec sign), degenerate Qed (real), shewchuk_orient alias, cross lemma; nts/orient_equiv/transport loud Admitted; outcome appended to pivot doc; accurate comments) | **Next**: IsEquiv + one transport Qed using B64_Expansion_Shewchuk + the formal lemmas. Update table on close. |
