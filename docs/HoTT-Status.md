...
| Shewchuk base (orient/expansion) | GREEN skeleton landed (PR #90) — fill in progress (feature/hott-rgr-shewchuk-proofs) | `theories-hott/ShewchukBaseEquiv.v` (Point + cross, Dir CCW/CW/COLLINEAR, formal/exact_orient now defined via cross sign + Rlt_dec, one real Qed degenerate lemma; shewchuk_orient/nts_orient/orient_equiv still Admitted (loud) for the linkage; review comment added) | **Fill real proofs**: turn the remaining Admitted into Qed using B64_Expansion_Shewchuk.v + sign_of..._correct_shewchuk + Orient_b64_exact/sound + Orientation.v invariants (one transport lemma). New branch started post-merge. |
