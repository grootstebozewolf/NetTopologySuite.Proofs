...
| Shewchuk base (orient/expansion) | GREEN (PR #90 skeleton + fill slice on feature/hott-rgr-shewchuk-fill-proofs): concrete formal_orient + degenerate Qed + cross_antisym + antisym Qed (transport example) | `theories-hott/ShewchukBaseEquiv.v` (Point + cross + cross_antisymmetric, Dir, formal_orient defined via Rlt/Rgt_dec, degenerate Qed, shewchuk_orient alias, formal_orient_antisym now Qed using cross_antisym (per review), cross lemma; nts/orient_equiv/transport still loud Admitted; outcome appended + table updated) | **Next**: IsEquiv for orient_equiv + one real transport Qed (using B64_Expansion_Shewchuk + formal lemmas). Update on close. |
...
Last updated: fill RGR slice close on feature/hott-rgr-shewchuk-fill-proofs (formal def + Qed + lemma + alias + outcome append + table refresh per process).
