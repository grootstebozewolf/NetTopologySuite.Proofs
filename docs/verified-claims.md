...
| `EdgeFaceBridge.v : dart_walk_endpoints_reachable` (+ `face_period_ge3_of_fan_nospur`, `same_face_twin_step_index`, `same_face_twin_first_step_index`, `same_face_twin_prefix_loop_E_minus`, `seg_properly_crosses_reversal`, `same_face_twin_carrier_exclusive_d`, `same_face_twin_disconnect`, `same_face_twin_is_cut`, `edge_2_connected_twins_sep`, `H_bridge_well_noded`) | **H_bridge combinatorial bridge (packaging landed, reach core Admitted):** rungs 1–3b-ii + 3b-iii carrier exclusivity/adj characterisations Qed; `same_face_twin_disconnect` → `H_bridge_well_noded` exported without Section Variables; residual axioms `not_reachable_E_minus_{dtip,dbase}_*` (general-fan `E_minus` endpoint avoidance). Contrast geometric H_bridge in `OverlayCorrectness` (JCT-gated) `[cond]` | 3 |
