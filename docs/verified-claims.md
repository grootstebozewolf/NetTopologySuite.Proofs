...
| `EdgeFaceBridge.v : dart_walk_endpoints_reachable` (+ `face_period_ge3_of_fan_nospur`, `same_face_twin_step_index`, `same_face_twin_first_step_index`, `same_face_twin_prefix_loop_E_minus`, `seg_properly_crosses_reversal`, `same_face_twin_carrier_exclusive_d`, `same_face_twin_disconnect`, `same_face_twin_is_cut`, `edge_2_connected_twins_sep`, `H_bridge_well_noded`) | **H_bridge combinatorial bridge (packaging landed, reach layer Admitted):** rungs 1–3b-ii + 3b-iii carrier exclusivity/adj/`not_adj_*` barrier Qed; `same_face_twin_disconnect` → `H_bridge_well_noded` exported without Section Variables; **(Rung 3b-v)** the reach layer is now a SINGLE named premise `H_bridge_core` (the combinatorial planar-bridge seam) — both reach-core lemmas, the outgoing-tip pair, and the capstones are all **Qed** on top of it, so `Print Assumptions H_bridge_well_noded` lists exactly `H_bridge_core` (+ standard classical/funext). Registry gap 2 → 1. Contrast geometric H_bridge in `OverlayCorrectness` (JCT-gated) `[cond]` | 3 |
