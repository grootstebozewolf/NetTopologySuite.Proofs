...
| `FaceOrbitSep.v : extract_faces_valid_sep` (+ `extract_faces_holes_valid_sep`) | **Capstones over one hypothesis:** both extractors validated from `well_noded_darts` + `no_spurs` + `twins_in_different_faces` — the per-face quantifier collapsed to a single global orbit condition (= no cut edge); the 2-edge-connected derivation of it is the deferred deeper rung `[cond]` | 3 |
