...
| `EdgeFaceBridge.v : dart_walk_endpoints_reachable` (+ `dart_on_walk_endpoints_adj`, `same_face_twin_both_on_period_walk`, `BridgePackaging.edge_2_connected_twins_sep`) | **H_bridge scaffold (in progress):** dart↔edge incidence, period-walk linkage, walk-level `reachable`; contrapositive packaging of H_bridge modulo open `same_face_twin_is_cut` (requires `no_spurs`) `[cond]` | 3 |
