...
| `EdgeFaceBridge.v : dart_walk_endpoints_reachable` (+ `face_period_ge3_of_fan_nospur`, `same_face_twin_step_index`, `same_face_twin_first_step_index`, `same_face_twin_prefix_loop_E_minus`, `is_cut_edge_of_dart_disconnect`, `same_face_twin_breaks_face_twin_free`, `dart_on_walk_endpoints_adj_E_minus`, `reachable_E_minus_from_dbase_singleton`, `same_face_twin_disconnect_singleton_out`, `SameFaceTwinCutCore.same_face_twin_is_cut`, `BridgePackaging.edge_2_connected_twins_sep`) | **H_bridge scaffold (in progress):** dart↔edge incidence, period-walk linkage, walk-level `reachable`, `is_cut_edge` packaging, rung-3b-i path layer (minimal twin index + `E_minus` prefix loop), rung-3b-ii singleton-fan disconnect base cases (`E_minus` reachable collapse + packaged negated reachability), rung-3b prep (`E_minus` walk adjacency + dumbbell/`face_twin_free` obstruction); `same_face_twin_is_cut` and H_bridge closure modulo open full `same_face_twin_disconnect` (requires `no_spurs`) `[cond]` | 3 |
