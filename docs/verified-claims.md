...
| `EdgeFaceBridge.v : dart_walk_endpoints_reachable` (+ `face_period_ge3_of_fan_nospur`, `same_face_twin_step_index`, `is_cut_edge_of_dart_disconnect`, `SameFaceTwinCutCore.same_face_twin_is_cut`, `BridgePackaging.edge_2_connected_twins_sep`) | **H_bridge scaffold (in progress):** dart↔edge incidence, period-walk linkage, walk-level `reachable`, `is_cut_edge` packaging; `same_face_twin_is_cut` and H_bridge closure modulo open `same_face_twin_disconnect` (requires `no_spurs`) `[cond]` | 3 |
