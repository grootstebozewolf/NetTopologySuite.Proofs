...
| `ConvexRayCrossing.v : convex_in_ring_iff_one_crossing` (+ `inc_cross_count_le_one`, `dec_cross_count_le_one`, `convex_ray_crosses_le_two`, `diamond_in_ring_iff_one_crossing`, `hexagon_in_ring_iff_one_crossing`) | **Convex-chain monotonicity, the y-modulator crossing bound — the crisp discrete Jordan characterisation for the convex case:** a convex / y-unimodal ring (one presented as a `bimonotone_split`) is crossed AT MOST TWICE by any rightward ray (`convex_ray_crosses_le_two` — each monotone chain at most once via `*_chain_le_one_cross`, summed over the split by `cross_count_app`), so combined with ray parity (`ray_parity_count`: `point_in_ring` ⟺ `Nat.odd (cross_count) = true`) a point is inside **iff the ray crosses exactly once** (`convex_in_ring_iff_one_crossing`: `point_in_ring p r ↔ cross_count p (ring_edges r) = 1`). Where the bare parity seam only fixes the crossing *parity*, convexity fixes the exact *count*. Validated on the diamond and hexagon (both already `bimonotone_split`). The geometric residual (convexity ⟹ a `bimonotone_split` exists, i.e. y-unimodal vertex order) is unchanged `[exact]` | 3 |
| `HotPixelConvexRing.v : pixel_point_in_ring_iff_box` (+ `pixel_ring_edges`, `pixel_bottom_no_cross`, `pixel_top_no_cross`, `pixel_right_crosses_iff`, `pixel_left_crosses_iff`, `pixel_ray_crosses_le_two`, `pixel_in_ring_iff_one_crossing`, `pixel_point_in_ring_implies_in_hot_pixel`, `in_hot_pixel_off_bottom_implies_point_in_ring`, `pixel_grazing_bottom_edge`) | **The hot pixel as a convex ring — snap-rounding meets ray-parity:** the half-open snap-rounding square `HotPixel.in_hot_pixel` (`[cx−r, cx+r) × [cy−r, cy+r)`, `r = hot_pixel_radius scale`) presented as a CCW `Ring` (`pixel_ring`) and connected to `Overlay.point_in_ring`. Its top/bottom edges are HORIZONTAL, so it is not a strict `bimonotone_split`, but a rightward ray never crosses a horizontal edge (`edge_crosses_ray` needs a strict y-straddle) — only the two VERTICAL edges count, each crossing iff the ray straddles the pixel and the origin is left of that edge's x. So the ring is crossed at most twice and a point is inside **iff crossed exactly once** (the `convex_in_ring_iff_one_crossing` pattern, reproved directly for the flat-edged square via `cross_count_cons_*` + `ray_parity_count`), giving the headline `point_in_ring p (pixel_ring C s)` ⟺ a **half-open-x / open-y box** (x∈`[cx−r,cx+r)`, y∈`(cy−r,cy+r)`). Bridges to `in_hot_pixel`: total above the bottom edge, with `pixel_grazing_bottom_edge` a concrete point on the *included* bottom edge that is `in_hot_pixel` yet not `point_in_ring` — the hot-pixel incarnation of the vertex-grazing caveat (`JCT_VertexGrazingCounterexample`). Validated on the unit pixel `[exact]` | 3 |
