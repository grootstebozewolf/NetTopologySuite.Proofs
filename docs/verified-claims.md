...
| `PassesThrough_b64_grid_exact.v : grid_ratio_gap_exceeds_ulp_band` | **C1 slice 14 — gap beats band (Qed):** the three bricks combine — for two distinct ratios `u=na/da`, `v=nb/db` in `[-1,1]` with `\|da\|,\|db\| ≤ 2²⁴`, `½ulp(round u)+½ulp(round v) < \|u−v\|` (band `≤ 2⁻⁵²` by slice 13, gap `≥ 1/(\|da\|\|db\|) ≥ 2⁻⁴⁸` by slice 12, `2⁻⁵² < 2⁻⁴⁸`). **Exactly `clip_separated`'s right disjunct** for the binding `(tmin_e,tmax_e)` pair — the quantitative heart of unconditional on-grid soundness for `\|n\| ≤ 2²³`; the residual is purely structural (exhibit the binding bounds as such ratios); Qed-closed `[full-b64]` | 4 |
