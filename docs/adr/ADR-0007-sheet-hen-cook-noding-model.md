...
| **ι interior circular×chord** | #707 @ `1dbc2c2` (joint gate); discharge `SidecarCircInteriorHit.v`; successor `SidecarCircMixedGate.v` | `SidecarCircMixedGate.v : ticket_0007_iota_mixed_gate_qed_or_qex` | `I_ok_mixed` Hit stays `mixed_joint_params` (`SidecarCircInterior.v : interior_mixed_hit_arm_missing`). Successor `I_ok_mixed_gate` = Joint (`I_ok_mixed`) + Interior (`I_ok_mixed_interior_arm`; `SidecarCircInteriorHit.v : locked_interior_I_ok_interior`). Do not drop the joint gate. Not host `I_ok` / Γ remint. |
...
| — | ι mixed-gate successor | `SidecarCircMixedGate.v : ticket_0007_iota_mixed_gate_qed_or_qex` | **QED** — `I_ok_mixed_gate` Joint = `I_ok_mixed` + Interior = `I_ok_mixed_interior_arm`; `I_ok_mixed` gate stands | `SidecarCircMixedGate.v : locked_joint_I_ok_mixed_gate`, `SidecarCircMixedGate.v : locked_interior_I_ok_mixed_gate` |
