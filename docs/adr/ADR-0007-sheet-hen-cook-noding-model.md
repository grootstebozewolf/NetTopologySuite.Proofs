...
| **ι interior circular×chord** | #707 @ `1dbc2c2` (joint gate); sibling `SidecarCircInteriorHit.v` | `SidecarCircInterior.v : ticket_0007_iota_gap_qed_or_qex` | **QEX** — missing ctor `InteriorMixedHitArm` = `I_ok_mixed` Hit ∧ `interior_span_params` (`SidecarCircInterior.v : interior_mixed_hit_arm_missing`). `I_ok_interior` does not discharge (`SidecarCircInteriorHit.v : iota_park_not_discharged_by_I_ok_interior`). Gate stays `mixed_joint_params`. Not host `I_ok` / Γ remint. |
