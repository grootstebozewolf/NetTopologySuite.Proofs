...
scope is chord–chord and circular–circular (`MkCirc`). Mixed and tags
Decline. Identity is structural (`ShareOne` / `MintTwo`).
...
named QEX stops (ι / ρ) instead of restaging interior-mixed /
bag-loop theater in each paragraph. Host CircGamma is discharged.
...
|------|--------|--------|-----------|
| **Γ CircGamma** | discharged (claimId `0007-gamma-mkcirc`) | `CircularCook.v : ticket_64_circ_gamma_qed_or_qex` | **QED** — host `MkCirc` inhabits `CircGammaMkCirc` (`CircularCook.v : circ_gamma_mkcirc_inhabits`); `circular_gamma_status = CircGammaDischarged` (`CircularCook.v : circular_gamma_is_discharged`). nlerp still misses the reflex principal span (`CircularCook.v : reflex_nlerp_misses_principal`) — not the remaining hole. Sidecar `arc_gamma` is not host Γ. |
...
| 4 | First cook scope completeness | `Adr0007NodingEpic.v : ticket_0007_qed_or_qex` | **QEX** — ellipse–ellipse missing (508 mirror); NURBS×NURBS is first cook | `SheetHenCook.v : ellipse_ellipse_not_first_scope`, `SheetHenCook.v : nurbs_egg_first_cook_scope` |
...
| — | Bag-level cook loop (term / confl on a leftover bag) | `Adr0007NodingEpic.v : ticket_0007_cook_term_qed_or_qex` | **QEX** — named 508-style gap: `LeftoverBagTermArm` missing; leftover_quad width conserved; kiss / share / mint not covered. CRV-TOUCH / `𝓘`-family; not a soft gap | `SheetHenCookLoop.v : leftover_bag_term_arm_missing`, `SheetHenCookLoop.v : leftover_quad_width_conserved` |
...
| — | CircularArc γ / CircGamma | `CircularCook.v : ticket_64_circ_gamma_qed_or_qex` | **QED** — host `MkCirc` angle interpolant; `CircGammaDischarged`; nlerp miss is not the remaining hole | `CircularCook.v : circ_gamma_mkcirc_inhabits`, `CircularCook.v : circular_gamma_is_discharged`, `CircularCookMkCirc.v : locked_mkcirc_I_ok` |
...
| — | Host circular IHit → `try_cook_hit` | `Adr0007NodingEpic.v : ticket_0007_circ_host_cook_qed_or_qex` | **QED** — MkCirc chickens mint; tags still None | `CircularCookMkCirc.v : cooked_mkcirc_try`, `SheetHenCook.v : try_cook_hit_circular_hit_none` |
| — | Circular Hit feeds sidecar `split(t)` | `CircularCookSplit.v : ticket_0007_circ_cook_step_qed_or_qex` | **QED** — locked plus-root leftovers meet at `p+`; CircGamma is CircGammaDischarged | `CircularCookSplit.v : cooked_circ_plus_try`, `CircularCookSplit.v : cooked_circ_plus_ok` |
...
| — | I.1 I_gloss / host CircGamma | `CircularCook.v : ticket_0007_i1_gloss_qed_or_qex` | **QED** — `I_gloss` is MkCirc Hit; tags still Decline | `CircularCook.v : circ_gamma_mkcirc_inhabits`, `CircularCookMkCirc.v : locked_mkcirc_I_ok`, `SheetHenCook.v : circular_decline_I_ok` |
...
| — | I.2 not arc membership | `CircularCookHit.v : ticket_0007_i2_arc_scope_qed_or_qex` | **QEX** — γ_full only; CircGamma is CircGammaDischarged; first cook stays chord–chord | `CircularCook.v : circular_gamma_is_discharged`, `CircularCook.v : circular_is_first_cook_scope` |
...
| — | I.3 not arc membership | `CircularCookEmpty.v : ticket_0007_i3_scope_qed_or_qex` | **QEX** — γ_full only; CircGamma is CircGammaDischarged; sidecar cook stays locked | `CircularCook.v : circular_gamma_is_discharged`, `CircularCook.v : circular_is_first_cook_scope` |
...
| — | I.8 not arc / not bag discharge | `CircularCookConfluence.v : ticket_0007_i8_scope_qed_or_qex` | **QEX** — γ_full only; CircGamma is CircGammaDischarged; bag loop stays QEX | `CircularCook.v : circular_gamma_is_discharged`, `SheetHenCook.v : cook_loop_is_obligation` |
...
| — | I.9 not CircGamma / not scope expand | `CircularCookLicense.v : ticket_0007_i9_scope_qed_or_qex` | **QEX** — CircGamma is CircGammaDischarged; first cook stays chord–chord | `CircularCook.v : circular_gamma_is_discharged`, `CircularCook.v : circular_is_first_cook_scope` |
...
**Decision:** Accepted by Joost (BDFL). Vocabulary law for CRV-TOUCH / RGR. Soft gaps (a)(b)(c) closed as checklist rows. Parks ι / ρ are landed named QEX; Γ CircGamma is CircGammaDischarged via MkCirc. Honest remaining opens (FP noder, Hobby, kiss) stay open and do not reopen Status.
...
- Parks ι / ρ — landed named QEX; Γ CircGammaDischarged via MkCirc (see Parks). Pairwise leftover-width and I.8 one-step confluence stay sibling QED. **Arc** cook termination is a sister card, not this Accept.
...
The *host* cook (`try_cook_hit`) later mints on `MkCirc` (see
Γ host MkCirc / CircGamma). This 2026-09-08 letter fed the
sidecar cook; tags still return None. Touch / kiss is a fenced
QEX arm, not a CRV-TOUCH kiss decision. Not a remint of
`ArcSplitAtNode`. Does not reopen Status.

| Stop | Arm | Lemma |
|------|-----|-------|
| `Adr0007NodingEpic.v : ticket_0007_circ_host_cook_qed_or_qex` | **QED** (later, MkCirc) — tags still None; MkCirc chickens mint | `CircularCookMkCirc.v : cooked_mkcirc_try`, `SheetHenCook.v : try_cook_hit_circular_hit_none` |
...
| `CircularCookSplit.v : ticket_0007_circ_cook_step_qed_or_qex` | **QED** — `I_circles_gamma` Hit cooks; CircGamma is CircGammaDischarged | `CircularCookSplit.v : cooked_circ_plus_try`, `CircularCookSplit.v : cooked_circ_plus_ok` |
...
| `CircularCookSplit.v : ticket_0007_circ_mint_two_qed_or_qex` | **QED** — Hit allocates `MintTwo`; both roots cook; CircGamma is CircGammaDischarged | `CircularCookSplit.v : cooked_circ_mint_two_try`, `CircularCookSplit.v : cooked_circ_mint_two_ok` |
...
| `CircularCook.v : ticket_0007_i1_gloss_qed_or_qex` | **QEX** — `I_gloss` undefined while CircGamma is QEX | `CircularCook.v : circular_gamma_is_discharged` |
...
| `CircularCookHit.v : ticket_0007_i2_arc_scope_qed_or_qex` | **QEX** — not arc membership; CircGamma is CircGammaDischarged | `CircularCook.v : circular_gamma_is_discharged` |
...
| `CircularCookEmpty.v : ticket_0007_i3_scope_qed_or_qex` | **QEX** — not arc membership; CircGamma is CircGammaDischarged | `CircularCook.v : circular_gamma_is_discharged` |
...
| `CircularCookConfluence.v : ticket_0007_i8_scope_qed_or_qex` | **QEX** — CircGamma is CircGammaDischarged; bag loop stays obligation | `CircularCook.v : circular_gamma_is_discharged` |
...
| `CircularCookLicense.v : ticket_0007_i9_scope_qed_or_qex` | **QEX** — CircGamma is CircGammaDischarged; first cook stays chord–chord | `CircularCook.v : circular_gamma_is_discharged` |
...
| `CircularCookClose.v : ticket_0007_i10_host_qed_or_qex` | **QEX** — CircGamma is CircGammaDischarged; first cook stays chord–chord | `CircularCook.v : circular_gamma_is_discharged`, `SheetHenCook.v : first_cook_scope_chord_chord` |
...
| `CircularCookSpanFilter.v : ticket_0007_ii1_host_qed_or_qex` | **QEX** — CircGamma is CircGammaDischarged; first cook stays chord–chord; span Hit ≠ host `I_ok` | `CircularCook.v : circular_gamma_is_discharged`, `CircularCookSpanFilter.v : ii1_span_hit_not_host_I_ok` |
...
| `CircularCookSpanSplit.v : ticket_0007_ii2_host_qed_or_qex` | **QEX** — CircGamma is CircGammaDischarged; first cook stays chord–chord; span Hit ≠ host `I_ok` | `CircularCook.v : circular_gamma_is_discharged`, `CircularCookSpanSplit.v : ii2_span_hit_not_host_I_ok` |
...
| `CircularCookOkCirc.v : ticket_0007_ii3_host_qed_or_qex` | **QEX** — CircGamma is CircGammaDischarged; first cook stays chord–chord; `I_ok_circ` Hit ≠ host `I_ok` | `CircularCook.v : circular_gamma_is_discharged`, `CircularCookOkCirc.v : ii3_I_ok_circ_hit_not_host_I_ok` |
...
| `CircularCookCloseII.v : ticket_0007_ii4_host_qed_or_qex` | **QEX** — CircGamma is CircGammaDischarged; first cook stays chord–chord | `CircularCook.v : circular_gamma_is_discharged`, `SheetHenCook.v : first_cook_scope_chord_chord` |
...
| `CircularCookCsConcat.v : ticket_0007_b1_host_qed_or_qex` | **QEX** — CircGamma is CircGammaDischarged; first cook stays chord–chord | `CircularCook.v : circular_gamma_is_discharged`, `SheetHenCook.v : first_cook_scope_chord_chord` |
...
| `CircularCookCcConcat.v : ticket_0007_b2_host_qed_or_qex` | **QEX** — CircGamma is CircGammaDischarged; first cook stays chord–chord; mixed Hit is not host `I_ok` | `CircularCook.v : circular_gamma_is_discharged`, `SheetHenCook.v : first_cook_scope_chord_chord` |
...
| `CircularCookCpConcat.v : ticket_0007_b3_host_qed_or_qex` | **QEX** — CircGamma is CircGammaDischarged; first cook stays chord–chord; mixed closing Hit is not host `I_ok` | `CircularCook.v : circular_gamma_is_discharged`, `SheetHenCook.v : first_cook_scope_chord_chord` |
...
| `SidecarCircMixed.v : ticket_0007_b_mixed_host_qed_or_qex` | **QEX** — CircGamma is CircGammaDischarged; first cook stays chord–chord; interior mixed cook parked; `I_ok_mixed` Hit ≠ host `I_ok` | `CircularCook.v : circular_gamma_is_discharged`, `SheetHenCook.v : first_cook_scope_chord_chord` |
...
| `SidecarCircBags.v : ticket_0007_b_bags_host_qed_or_qex` | **QEX** — CircGamma is CircGammaDischarged; first cook stays chord–chord; host mixed `I_ok` Decline; interior mixed cook parked | `CircularCook.v : circular_gamma_is_discharged`, `SheetHenCook.v : first_cook_scope_chord_chord` |
...
3. Host mixed `I_ok` Hit stays False; first cook stays
   chord–chord. Host CircGamma is CircGammaDischarged.
...
| `SidecarCircInterior.v : ticket_0007_iota_host_qed_or_qex` | **QEX** — CircGamma is CircGammaDischarged; first cook stays chord–chord; host mixed `I_ok` Decline | `CircularCook.v : circular_gamma_is_discharged`, `SheetHenCook.v : first_cook_scope_chord_chord` |
...
### Letter after Accept — Γ host MkCirc / CircGamma (2026-09-11)

Host CircGamma (Γ) is the 3-axiom interpolant `γ : [0,1] → S` on
`CircularEgg` via `MkCirc` (`θ₀ + t·Δθ`; angles are egg data).
This letter flips `circular_gamma_status` to `CircGammaDischarged`
...
— that is a fact about nlerp, not the remaining hole.

1. `Egg` carries `MkCirc` (`CircularCook.v : circ_gamma_mkcirc_inhabits`,
   `CircularCookMkCirc.v : circular_egg_mkcirc_or_tag`). Tags remain
   `MkOutOfScope` and still Decline.
2. Host γ is atan2-free. Chord-project nlerp is not that interpolant
   (`CircularCook.v : reflex_nlerp_misses_principal`).
3. `first_cook_scope` includes circular–circular as part of this
   CircGamma + MkCirc Hit discharge. Mixed stays Decline.
   ADR-0007 stays Accepted.

| Stop | Arm | Lemma |
|------|-----|-------|
| `CircularCook.v : ticket_64_circ_gamma_qed_or_qex` | **QED** — `MkCirc` inhabits; `CircGammaDischarged`; first cook includes circular–circular | `CircularCook.v : circ_gamma_mkcirc_inhabits`, `CircularCook.v : circular_gamma_is_discharged` |
| `CircularCook.v : ticket_0007_i1_gloss_qed_or_qex` | **QED** — `I_gloss` is MkCirc Hit; tags still Decline | `CircularCookMkCirc.v : locked_mkcirc_I_ok`, `SheetHenCook.v : circular_decline_I_ok` |
| `CircularCook.v : ticket_0007_gamma_nlerp_qed_or_qex` | **QED** — CircGamma discharged; nlerp miss remains a nlerp fact | `CircularCook.v : circ_gamma_mkcirc_inhabits`, `CircularCook.v : reflex_nlerp_misses_principal` |
| `Adr0007NodingEpic.v : ticket_0007_circ_host_cook_qed_or_qex` | **QED** — `try_cook_hit` mints on the locked MkCirc pair | `CircularCookMkCirc.v : cooked_mkcirc_try` |

Witness: `0007-gamma-mkcirc`. Prior Parks witness: `0007-Gamma-circgamma`.
Status stays **Accepted**. Parks ι / ρ remain.
...
1. `LeftoverBagTermArm` does not inhabit. `leftover_width` is
...
| Stop | Arm | Lemma |
|------|-----|-------|
| `Adr0007NodingEpic.v : ticket_0007_cook_term_qed_or_qex` | **QEX** — `LoopObligation`; `LeftoverBagTermArm` missing; leftover_quad width conserved; pairwise split stays sibling QED | `SheetHenCookLoop.v : leftover_bag_term_arm_missing`, `SheetHenCookLoop.v : leftover_quad_width_conserved` |
| `SheetHenCookLoop.v : ticket_0007_rho_gap_qed_or_qex` | **QEX** — named missing constructor + conserved bag-sum | `SheetHenCookLoop.v : leftover_bag_term_arm_missing` |
...
| `SheetHenCookLoop.v : ticket_0007_rho_scope_qed_or_qex` | **QEX** — leftover_quad is one Hit-split; arc term stays sister; bag loop stays obligation; ellipse stays out | `SheetHenCookLoop.v : leftover_quad_is_one_hit`, `SheetHenCookLoop.v : arc_cook_term_is_sister` |
...
| `NodingNG.v : ticket_0007_nodingng_rho_qed_or_qex` | **QEX** — NodingNG chord is pairwise / one-step, not `LoopDischarged`; Parks ρ | `SheetHenCookLoop.v : leftover_bag_term_arm_missing`, `SheetHenCookLoop.v : leftover_quad_width_conserved` |
| `NodingNG.v : ticket_0007_nodingng_scope_qed_or_qex` | **QEX** — NodingNG stays pairwise/one-step; host first cook includes chord/circular/clothoid/NURBS; ellipse stays out | `SheetHenCook.v : first_cook_scope_chord_chord`, `SheetHenCook.v : circular_egg_first_cook_scope`, `SheetHenCook.v : nurbs_egg_first_cook_scope` |
...
**QEX.** Host CircGamma is CircGammaDischarged. First cook stays chord–chord.
...
| `SidecarCircInteriorHit.v : ticket_0007_iota_interior_host_qed_or_qex` | **QEX** — CircGamma is CircGammaDischarged; first cook stays chord–chord; host mixed `I_ok` Decline; host interior cook parked | `CircularCook.v : circular_gamma_is_discharged`, `SheetHenCook.v : first_cook_scope_chord_chord` |
...
(`nurbs_decline_I_ok`) and packages them. Prefer `SidecarNurbs*`
for the tag-packaging face. Existing NURBS corpus is metric /
length research — package what is already Qed; do not remint
#508 length / Cox-de-Boor as noding. One locked fixture. Do not
ship a NURBS×NURBS noder or Campaign I–II.
...
Golden quarter stays a metric cite. Host `MkNurbs` + NURBS×NURBS
first cook are the later letter `0007-nurbs-first-cook`.

**QEX parks.** Campaign I–II / length-as-noding / Cox-de-Boor /
bag loop. Hit-arm / first-cook expand are discharged QED by
claimId `0007-nurbs-first-cook`. Do not fake `LoopDischarged`.
Parks ι / ρ cited once (ι row already records #717 discharge).
...
| `SidecarNurbsEgg.v : ticket_0007_nurbs_not_first_cook_qed_or_qex` | **QED** — NURBS×NURBS is first cook; `MkNurbs` / Hit-arm / first-cook expand inhabit | `SheetHenCook.v : nurbs_egg_first_cook_scope`, `SidecarNurbsEgg.v : sidecar_nurbs_mknurbs_inhabits`, `SidecarNurbsEgg.v : sidecar_nurbs_hit_arm_inhabits` |
| `SidecarNurbsEgg.v : ticket_0007_nurbs_parks_qed_or_qex` | **QEX** — Campaign I–II / length-as-noding / Cox-de-Boor / bag loop parked; Parks ι / ρ | `SidecarNurbsEgg.v : sidecar_nurbs_letter_is_first_cook_expanded`, `SheetHenCook.v : cook_loop_is_obligation` |
...
missing constructors: no `MkCirc` on `Egg`; CircGamma is CircGammaDischarged
...
| `SidecarCircEgg.v : ticket_0007_circle_not_first_cook_qed_or_qex` | **QEX** — circular×circular stays out of first cook; `MkCirc` / CircGamma / Hit-arm / first-cook-expand missing; cite Parks Γ | `SheetHenCook.v : circular_egg_first_cook_scope`, `SidecarCircEgg.v : sidecar_circ_egg_mkcirc_missing`, `SidecarCircEgg.v : sidecar_circ_egg_circgamma_missing` |
...
| `SidecarGeodesicEgg.v : ticket_0007_geodesic_not_first_cook_qed_or_qex` · planar machine `IntakeGeodesicCook.v : ticket_0007_geodesic_cook_qed_or_qex` | **QEX** as a class — geodesic×geodesic stays out of first cook; `MkGeodesic` / Hit-arm missing. **QED** as the planar machine — on \(S\) the geodesic is the chord: well-formed GEODESICSTRING is a function to the LINESTRING bag (`IntakeGeodesicCook.v : ticket_0007_geodesic_intake_fn_qed_or_qex`), the locked bow-tie polyline cooks by host `I_ok` + `try_cook_hit` on chord×chord, one fresh hen, leftovers meet, τ = LINESTRING; famous nets are chord bags (`IntakeGeodesicCook.v : ticket_0007_geodesic_famous_chords_qed_or_qex`); ambient manifold / wire stay **QEX** with closed terms (`IntakeGeodesicCook.v : ticket_0007_geodesic_ambient_qed_or_qex`, `IntakeGeodesicCook.v : ticket_0007_geodesic_wire_qed_or_qex`); scope not expanded (`IntakeGeodesicCook.v : ticket_0007_geodesic_scope_qed_or_qex`) | `SidecarGeodesicEgg.v : geodesic_geodesic_not_first_scope`, `SidecarGeodesicEgg.v : sidecar_geodesic_mkgeodesic_missing`, `SidecarGeodesicEgg.v : sidecar_geodesic_hit_arm_missing`, `IntakeGeodesicCook.v : gx_cross_hit`, `IntakeGeodesicCook.v : gx_try_cook_some`, `IntakeGeodesicCook.v : sheet_chord_misses_pole` |
...
NURBS×NURBS is first cook (`0007-nurbs-first-cook`). SIN / ellipse /
spiral / geodesic stay out of first cook.
...
Other egg first-cooks. Completeness of first cook (ellipse still
missing).
...
### Letter after Accept — NURBS×NURBS first-cook (2026-09-12)

Needle AFTER clothoid first-cook (#728 @ `14cdae8`). claimId
`0007-nurbs-first-cook`. Constructor + first cook together (unlike
clothoid, which was split because `MkClothoid` was ordered first).
One host `MkNurbs` on `Egg` (parallel to `MkCirc` / `MkClothoid`).
Put `EggNurbs × EggNurbs` in `first_cook_scope`. `MkNurbs` pairs
are `interpolant_pair`. Locked crossing pair inhabits host `IHit`
via `on_nurbs` (Cox-de-Boor-free chord-parameter interpolant; eggs
keep an off-chord control point; `nurbs_split` mints `MkNurbs`
children). `try_cook_hit` returns `Some`. Thin siblings
`SheetHenNurbsEgg.v` + `NurbsCookMkNurbs.v` — `SheetHenCook.v`
stays at the module-split ceiling.

Not a silent `I_ok` demote to `on_chord`. Not length / Cox-de-Boor
/ knot-span as the noding engine. Mixed NURBS×chord stays Decline.
SIN / ellipse / spiral / geodesic stay out of first cook.
Tags (`MkOutOfScope EggNurbs`) stay Decline. No new oracle keyword
(ADR-0006).

**QED.** `first_cook_scope EggNurbs EggNurbs`. Locked
`I_ok (MkNurbs _) (MkNurbs _) (IHit …)`. `try_cook_hit` mints.
Sidecar `MkNurbs` / Hit-arm / first-cook expand inhabit.

**QEX.** Length-as-noding / Cox-de-Boor / Campaign / ρ. Mixed
NURBS×chord. Other egg first-cooks. Completeness of first cook
(ellipse still missing).

Status stays **Accepted**. ADR-0006 Status stays **Accepted**.

| Stop | Arm | Lemma |
|------|-----|-------|
| `NurbsCookMkNurbs.v : ticket_0007_nurbs_first_cook_qed_or_qex` | **QED** — scope + locked Hit + mint; tags / mixed Decline; ellipse / ρ parked | `NurbsCookMkNurbs.v : locked_mknurbs_I_ok`, `NurbsCookMkNurbs.v : cooked_mknurbs_try`, `SheetHenCook.v : nurbs_egg_first_cook_scope` |
| `SidecarNurbsEgg.v : ticket_0007_nurbs_not_first_cook_qed_or_qex` | **QED** — `MkNurbs` / Hit-arm / first-cook expand inhabit | `SidecarNurbsEgg.v : sidecar_nurbs_mknurbs_inhabits`, `SidecarNurbsEgg.v : sidecar_nurbs_hit_arm_inhabits`, `SidecarNurbsEgg.v : sidecar_nurbs_first_cook_expand_inhabits` |
| `SidecarNurbsEgg.v : ticket_0007_nurbs_parks_qed_or_qex` | **QEX** — Campaign / length-as-noding / Cox-de-Boor / ρ parked | `SidecarNurbsEgg.v : sidecar_nurbs_not_length_noding`, `SheetHenCook.v : cook_loop_is_obligation` |
| `Adr0007NodingEpic.v : ticket_0007_qed_or_qex` | **QEX** — completeness still misses ellipse–ellipse | `SheetHenCook.v : ellipse_ellipse_not_first_scope` |

Witness: `0007-nurbs-first-cook`. Status stays **Accepted**. Parks ι / ρ. Γ is discharged (#724). This letter is first cook only, not a noder and not length / Cox-de-Boor as noding.
