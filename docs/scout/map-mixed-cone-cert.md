...
The live cex is an unnamed CCW pair (not leftover `Ⅵ`). Leftover
`Ⅴ` itself is QED (`RelateNGTouchMixedCone.v : leftover_v_qed_or_qex`).
...
(fill still `UNSUPPORTED`). Decline golden is the unnamed CCW pair
