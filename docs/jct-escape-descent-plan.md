...
| 5 | **Descent recursion + assembly**: each walk step strictly lowers the current height through the finite set of vertex levels (every wall's bottom vertex lies below the walker), so the walk reaches a count-free point in at most `|vertices|` rounds; `escape_descent_holds : ring_taut r -> ring_closed r -> ... -> escape_descent r`, composed through `parity_seam_offring_of_descent` — H1 closed | open |
