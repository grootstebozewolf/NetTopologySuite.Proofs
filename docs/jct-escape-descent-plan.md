...
| 5c-14 | **Cycle recursion + entry + payoff + assembly**: the cycle walk (iterate `hug_step_corner` along `cyclic_next`, reaching every edge from any start); the walk's entry (east approach → `hugs_west` of the first wall); the eastmost-corner payoff (a hug of an eastmost-vertex edge yields a connected point with count `0 < n`, or a parity contradiction on the pinched side); `escape_descent_holds` → `parity_seam_offring_of_descent` — H1 closed | open |
