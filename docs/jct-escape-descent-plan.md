...
| 5c-9 | **Remaining composites + traversal + assembly**: the three mirrors of the open-side min step (ew via xmir, max versions via ymir), the pinched-side turnaround (wedge jog: except-one corridor clearance + corner disk), the cycle recursion (cycle index as measure), the eastmost-corner parity dichotomy (pinched visit ⇒ count 1 ⇒ odd, contradicting the even walker), and `escape_descent_holds` → `parity_seam_offring_of_descent` — H1 closed | open |
