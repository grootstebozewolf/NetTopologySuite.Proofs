...
| 5c-15 | **The payoff + assembly**: from `hugs` of an eastmost-vertex edge, harvest a connected point with count `0 < n` (east-side hug: the corridor point east of `xsup`; west-side hug at the eastmost edge: count 1, odd — parity contradiction for the even walker); `escape_descent_holds : ring_taut → ring_core_nodup → no_horizontal_edges → escape_descent r`; compose through `parity_seam_offring_of_descent` — H1 closed | open |
