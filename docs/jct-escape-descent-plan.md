...
| 5c-2 | **The traversal + assembly**: boundary-hugging walk through the four corner moves (original + three mirrors), aimed at the eastmost vertex's sector — count there is `0 < n`; wall-switching/backing-out covered by the four-orientation kit; `escape_descent_holds : ring_taut r -> ring_closed r -> ... -> escape_descent r`, composed through `parity_seam_offring_of_descent` — H1 closed | open |
