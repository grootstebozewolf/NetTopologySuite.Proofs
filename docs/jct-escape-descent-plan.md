...
| 5c-13 | **The per-corner total step + cycle recursion + assembly**: `hugs := hugs_west ∨ hugs_east`; the total dispatcher (corner type by height signs, side by the bridge `Rle_dec`); the cycle walk (advance one edge per step, `cyclic_next`); the eastmost-corner parity dichotomy (pinched visit ⇒ count 1 ⇒ odd, contradicting the even walker); `escape_descent_holds` → `parity_seam_offring_of_descent` — H1 closed | open |
