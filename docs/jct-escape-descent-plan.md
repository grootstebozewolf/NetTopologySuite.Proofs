...
| 5c-12 | **Pinched mirrors + traversal + assembly**: the three mirrors of the pinched step; the per-corner total step (decidable trichotomy: pass-through / open / pinched by edge directions and `Rle_dec` on the carrier comparisons); the cycle recursion (cycle index as measure); the eastmost-corner parity dichotomy (pinched visit ⇒ count 1 ⇒ odd, contradicting the even walker); `escape_descent_holds` → `parity_seam_offring_of_descent` — H1 closed | open |
