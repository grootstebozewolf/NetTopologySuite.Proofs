...
| 5c-16b | **The payoff rides** (`JCTPayoffRides.v`): `payoff_east_ride` (count 0 east of everything), `payoff_west_ride_pt` (count 1), `payoff_passage_min` (count 0 below an open minimum), `payoff_wedge_min` (count 1 in a pinched wedge) — every hug configuration at the eastmost vertex pays off | **Qed** |
| 5c-17 | **THE CLOSING RUNG** (`JCTEscapeDescentHolds.v`): `escape_descent_holds : ring_taut → ring_core_nodup → no_horizontal_edges → escape_descent r` (entry → cycle hug → eastmost dispatch → count 0/1 → parity elimination), and `parity_seam_offring_taut` — **H1 CLOSED** for taut, proper, horizontal-free rings, three axioms exactly | **Qed** |

**THE CAMPAIGN IS COMPLETE.** The corrected polygonal Jordan seam `parity_characterises_interior_cont_offring` is a theorem (`parity_seam_offring_taut`) for every taut, proper (`ring_core_nodup`), horizontal-free closed ring: the trapped half and separation clause (PR #165) plus the escape half (this branch, rungs 1–5c-17) — no stub, no named hypothesis, no `Admitted`, exactly the three allowed axioms. Documented residual generality: `ring_taut` (strictly stronger than corpus `ring_simple`, which admits corridor-blocking T-touches), `ring_core_nodup` (distinct core vertices), and `no_horizontal_edges` (standard general position; a rotation argument could remove it). |
