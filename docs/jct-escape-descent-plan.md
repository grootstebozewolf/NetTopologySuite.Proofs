...
| 5c-7 | **Remaining composites + traversal + assembly**: the mirrored pass-through steps (up/east via the state's mirror transport), the extremum-corner steps (open side: passage + tip crossing + boxes; pinched side: the wedge turnaround, needing the except-one corridor clearance and the corner disk), the cycle recursion (cycle index as measure), the eastmost-corner parity dichotomy (pinched visit ⇒ count 1 ⇒ odd, contradicting the even walker), and `escape_descent_holds` → `parity_seam_offring_of_descent` — H1 closed | open |
