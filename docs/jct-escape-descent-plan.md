...
| 4b | **Clearances from simplicity + corners + recursion**: derive each corridor clearance's sign hypothesis from `ring_simple` touch-freedom (`crossings_distinct` style), choose the corner sector at each shared vertex, and recurse around the boundary (edge-count termination) until a count-free point is reached | open |
