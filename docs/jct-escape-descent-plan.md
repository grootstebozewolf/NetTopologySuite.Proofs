...
| 5c-5 | **The traversal + assembly**: boundary-hugging walk chaining passages, tip crossings, and box glue along the edge cycle, aimed at the eastmost vertex's sector — count there is `0 < n`; per-corner the right move exists (degree-2 corners under tautness: at a pass-through corner the open-side condition holds vacuously for the continuing side; at extremum corners the tip crossing switches sides); `escape_descent_holds : ring_taut r -> ring_closed r -> ... -> escape_descent r`, composed through `parity_seam_offring_of_descent` — H1 closed | open |
