...
| 5c-16 | **The payoff + assembly**: from `hugs` of an eastmost-vertex edge, harvest a connected guarded point with count 0 (east-side rounding lands east of all vertices, `ho_count_zero_east_ub`) or count 1 (band/wedge points, `ho_count_one_in` — odd, contradicting the even walker via `parity_constant_on_components`); `escape_descent_holds : ring_taut → ring_core_nodup → no_horizontal_edges → ring_closed → escape_descent r`; compose through `parity_seam_offring_of_descent` — H1 closed | open |
