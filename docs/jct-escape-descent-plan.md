...
| 5c-3 | **The traversal + assembly**: boundary-hugging walk chaining the four passage moves along the edge cycle, aimed at the eastmost vertex's sector — count there is `0 < n`; per-corner the right move exists (at each vertex of a taut ring, one of the four open-side conditions holds for the hugged wall, or the walk switches walls); `escape_descent_holds : ring_taut r -> ring_closed r -> ... -> escape_descent r`, composed through `parity_seam_offring_of_descent` — H1 closed | open |
