...
| 5c-6 | **The traversal + assembly**: boundary-hugging walk chaining passages, tip crossings, and box glue along the edge cycle (cycle index as the measure), aimed at the eastmost vertex's corner — an east-side pass there has count `0 < n`, and a pinched-side visit has count 1, contradicting the walker's even parity (so the even walker always rounds the open side); `escape_descent_holds : ring_taut r -> ring_core_nodup r -> no_horizontal_edges r -> ... -> escape_descent r`, composed through `parity_seam_offring_of_descent` — H1 closed | open |
