...
| 5c-11 | **Pinched turnaround composite + traversal + assembly**: the wedge descent/jog/ascent using the disk + fixed 4b-2 windows + pointwise incident clearances; the cycle recursion (cycle index as measure); the eastmost-corner parity dichotomy (pinched visit ⇒ count 1 ⇒ odd, contradicting the even walker); `escape_descent_holds` → `parity_seam_offring_of_descent` — H1 closed | open |
