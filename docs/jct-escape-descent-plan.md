...
| 5b | **Descent recursion + assembly**: choose the wall so `corner_opens_east` holds at its bottom corner (wall-switch argument when the companion ascends west); each round strictly lowers the height through the finite set of vertex levels, reaching a count-free point in at most `|vertices|` rounds; `escape_descent_holds : ring_taut r -> ring_closed r -> ... -> escape_descent r`, composed through `parity_seam_offring_of_descent` — H1 closed | open |
