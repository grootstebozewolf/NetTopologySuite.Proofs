...
| 5c-17 | **The dispatch + H1**: the eastmost corner pair (`vertex_out_edge` + `cyclic_prev`), the per-type/side dispatch selecting a ride (maxima via the y-flip payoff transport), the count-1 parity elimination (`parity_constant_on_components`), `escape_descent_holds : ring_taut → ring_core_nodup → no_horizontal_edges → escape_descent r`, composed through `parity_seam_offring_of_descent` — H1 closed | open |
