...
2. **PIVOT to chunked small-equivalence approach, in this risk/cost-optimal order** (building on Voronoi pilot + previous RGR decision).
   ...
   **Why Shewchuk first?** Shewchuk expansions are the common foundation for virtually every robust predicate in NTS (orientation, intersection, incircle for Delaunay/Voronoi, arc-aware variants). The classical work already has a lot of Qed infrastructure in the b64 layer and small-regime exactness results. Starting here gives the highest leverage for later chunks (Hobby noding depends on predicates; curve arcs reuse orient/incircle) at the lowest risk (we can do tiny, regime-restricted equivalences first and transport the existing classical proofs via univalence). It also directly benefits the current NTS `.Curve` and Robust* code paths that consumers actually use today.
