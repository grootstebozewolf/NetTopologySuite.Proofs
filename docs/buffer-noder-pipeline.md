...
| 2a offset @ d | parallel curve at distance `d` | 🟡 | segment soundness ✅ `BufferOffset.{offset_seg_dir,offset_point_dist,offset_perp_dist_to_line}`; arc soundness ✅ `ArcOffset.{arc_offset_dist_exact,arc_offset_no_kink,arc_offset_tangent_dot}` (concentric `r+d`, parallel-curve property exact up to the `d = -r` singularity, with a Qed failure witness past it); SQL/MM three-point closure ✅ `ArcOffsetThreePoint.arc_offset_preserves_arc` (offset of a valid `CircularArc` is a valid `CircularArc`, same center, radius `r+d`); segment-wise ring map ✅ `CurveRingOffset.{curve_ring_offset_arcs_valid,arc_join_offset_continuous}` + the S-curve tear witness `tangent_continuity_insufficient_for_offset` (G1-with-consistent-normals joins need no join edges; tangent-line continuity alone provably tears); ring-level capstone ✅ `CurveRingOffset.curve_ring_offset_valid` (smooth compound ring offset within the safety bound stays a `valid_curve_ring`); join/cap emission and the total ring assembly are in row 2b |
| 2b joins | round/miter/bevel at corners | 🟡 | round-join central angle ✅ `BufferJoin.corner_arc_sweep_eq_turn(_unit)` (Roadmap target 6); miter apex + limit cap + half-angle ✅ `BufferMiter.*` / `BufferMiterAngle.miter_cap_iff_sin_half` (JTS#180); bevel chord ✅ `BufferBevel.bevel_length_sq_sin_half`; curve-side round join EMITTED as a valid SQL/MM arc ✅ `CurveRoundJoin.{round_join_arc_valid,round_join_arc_center_radius,round_join_connects}` (circumcircle exactly `(P,\|d\|)`, splices onto the offset segments); spliced ring assembly ✅ `CurveOffsetAssembly.curve_ring_offset_round_valid` (round-join arcs inserted at non-G1 joins incl. the closing join; valid ring out, conditional only on the G1-decision spec); U-turn joins ✅ + curved round endcaps ✅ `CurveSemicircle.{semicircle_uturn_connects,semicircle_cap_connects}` (one semicircle construction, valid SQL/MM arc, circumcircle exactly `(P,\|d\|)`); total assembly ✅ `CurveOffsetAssemblyTotal.curve_ring_offset_total_valid` (three-way join policy, NO join exclusions: any valid compound ring offsets to a valid ring); SQL/MM hierarchy lift ✅ `CurvePolygonOffset.curve_{polygon,geometry}_offset_valid` (validity preserved ring → CurvePolygon → CurveGeometry, hole/polygon counts preserved); emitted edge ring CLOSES ✅ `CurveOffsetEmit.offset_emit_ring_closed` (+ all-chord/linear specialisation; chord offsets = `BufferOffset.offset_seg` definitionally); BEVEL assembly ✅ `CurveBevelJoin.curve_ring_offset_bevel_valid` (chord joins across every non-G1 tear incl. U-turns, leanest hypothesis set, all-chord in → all-chord out); MITER assembly ✅ `CurveMiterJoin.curve_ring_offset_miter_valid` (two chords through `BufferMiter.miter_apex` at chord-chord joins, perp-dist-`d` soundness attached, bevel fallback at arc joins) — the 2b join-emission story (round/bevel/miter) is COMPLETE |
| 2c endcaps | round/flat/square at line ends | 🟡 | ✅ `BufferEndcap.*` — flat (`flat_cap_length_sq`/`_perp_edge`), round (`round_cap_endpoints_on_circle`/`_apex_on_circle`), square (`square_cap_extension`/`square_cap_corner_dist_sq`) defining geometry (JTS#739/#1028); curve-side round cap EMITTED as a valid SQL/MM arc ✅ `CurveSemicircle.semicircle_cap_connects` (+d boundary to −d boundary around the line end); ring-level emitted edge lists close via `CurveOffsetEmit` (row 2b); OPEN-CHAIN two-sided cap walk ✅ `CurveCapWalk.curve_chain_buffer_valid` (left +d walk, semicircle caps, reversed −d walk → a CLOSED valid compound ring) |
...
`equidistant_point_is_arc_center`.  The remaining bricks (rungs 3–8)
landed the same week: segment-wise ring offset with the G1/tear join
dichotomy (`CurveRingOffset.v`), the round join and semicircle
(U-turn join + round endcap) emitted as valid SQL/MM arcs
(`CurveRoundJoin.v`, `CurveSemicircle.v`), and the assembly walks
(`CurveOffsetAssembly.v`, `CurveOffsetAssemblyTotal.v`) whose headline
`curve_ring_offset_total_valid` offsets ANY valid compound ring to a
valid compound ring with no join exclusions.  What still rides on
Option B / §6: `CurvePolygon`-level topology (hole/shell relations
under offset) and the point-set Minkowski semantics of §3.
